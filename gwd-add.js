...
`
			...
			return `
				...
				<script>
					(function(){
						...
						let ectimer = setInterval(function(){
		...
		function opWind(purl, pisweb, pname, phml) {
			...
			if (pisweb == 0) {
				...
				document.querySelector("body").insertAdjacentHTML("afterend", "<div id='whatdoyousay'></div>");
				// MARK 注意这里出现了echarts和polyfill不兼容的问题 以后修复：单独在另一个窗口打开是没有问题的
				acjq("#whatdoyousay").html(phml);
			}
		}
		if( localStorage.getItem("gwdang-fp") == null) {
		// if(true){
			// 首次加载数据
			initGWD();
		}else{
